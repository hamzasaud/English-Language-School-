...
export default function Footer() {
  ...
  const footerLinks = [
    {
      title: content.navigation[currentLocale].courses,
      links: content.courses.map(course => ({
        name: getLocalizedText(course.title, currentLocale),
        href: `/courses/${course.slug}`
      }))
    },
    {
      title: currentLocale === 'id' ? 'Informasi' : 'Information',
      links: [
        ...
        { name: currentLocale === 'id' ? 'Syarat & Ketentuan' : 'Terms & Conditions', href: '/terms' },
        { name: currentLocale === 'id' ? 'Kebijakan Privasi' : 'Privacy Policy', href: '/privacy' }
      ]
    }
  ];
    ...
    <footer className="bg-gray-900 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
              ...
              <div className="space-y-2 text-gray-300">
                <div>{getLocalizedText(content.settings.businessHours[currentLocale].weekdays, currentLocale)}</div>
                <div>{getLocalizedText(content.settings.businessHours[currentLocale].saturday, currentLocale)}</div>
                <div>{getLocalizedText(content.settings.businessHours[currentLocale].sunday, currentLocale)}</div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-lg font-semibold mb-4">
                {currentLocale === 'id' ? 'Ikuti Kami' : 'Follow Us'}
              </h3>
        ...
        <div className="mt-8 pt-8 border-t border-gray-800 text-center text-gray-400">
          <p>
            © {currentYear} English School Indonesia. {currentLocale === 'id' ? 'Semua hak dilindungi.' : 'All rights reserved.'}
          </p>
        </div>
      </div>
    </footer>
